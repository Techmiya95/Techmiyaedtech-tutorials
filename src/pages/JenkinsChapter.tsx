...
const CodeBlock = ({ code, title, output }: { code: string; title?: string; output?: string }) => {
    ...
    return (
        <div className="my-6 rounded-xl overflow-hidden shadow-lg border border-gray-700/50" style={{ background: "#1a1b26" }}>
            ...
            <SyntaxHighlighter language="groovy" style={customStyle} showLineNumbers={true} wrapLines={true}>
                {code}
            </SyntaxHighlighter>
        </div>
    );
...
const SectionContent = ({ section }: { section: Section }) => {
    return (
        <div className="mb-10">
            ...
            {section.note && (
                <div className="mt-4 bg-red-50 border-l-4 border-red-500 rounded-r-lg p-4 flex gap-3">
...
const JenkinsChapter = () => {
    const { slug } = useParams<{ slug: string }>();
    const [sidebarOpen, setSidebarOpen] = useState(false);
    const chapter = jenkinsChapters.find((ch) => ch.slug === slug);
    const chapterIndex = jenkinsChapters.findIndex((ch) => ch.slug === slug);
    const prevChapter = chapterIndex > 0 ? jenkinsChapters[chapterIndex - 1] : null;
    const nextChapter = chapterIndex < jenkinsChapters.length - 1 ? jenkinsChapters[chapterIndex + 1] : null;
            ...
            <div className="bg-gray-50 border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
                    <nav className="flex items-center gap-2 text-sm text-gray-500">
                        ...
                        <ChevronRight className="w-3.5 h-3.5" />
                        <Link to="/devops/jenkins" className="hover:text-red-600 transition-colors">Jenkins</Link>
                        <ChevronRight className="w-3.5 h-3.5" />
            ...
            <div className="max-w-7xl mx-auto flex">
                    ...
                    <div className="p-4">
                        <nav className="space-y-0.5">
                            {jenkinsChapters.map((ch) => (
                                <Link key={ch.id} to={`/devops/jenkins/${ch.slug}`} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm ${ch.slug === slug ? "bg-red-50 text-red-800 font-semibold" : "text-gray-600 hover:bg-gray-50"}`}>
                                    <span className={`w-6 h-6 rounded text-xs flex items-center justify-center font-bold ${ch.slug === slug ? "bg-red-600 text-white" : "bg-gray-100"}`}>{ch.id}</span>
                ...
                <main className="flex-1 min-w-0 px-4 sm:px-8 lg:px-12 py-8 lg:py-10">
                    ...
                    <div className="mt-12 pt-8 border-t border-gray-200 flex justify-between gap-4">
                        {prevChapter && <Link to={`/devops/jenkins/${prevChapter.slug}`} className="flex items-center gap-3 px-5 py-3 rounded-xl border border-gray-200 hover:border-red-300 transition-all">
                            <ChevronLeft className="w-5 h-5 text-gray-400" />
                            <div><p className="text-xs text-gray-400">Previous</p><p className="font-semibold text-gray-700">{prevChapter.title}</p></div>
                        </Link>}
                        {nextChapter && <Link to={`/devops/jenkins/${nextChapter.slug}`} className="flex items-center justify-end gap-3 px-5 py-3 rounded-xl border border-gray-200 hover:border-red-300 transition-all text-right">
