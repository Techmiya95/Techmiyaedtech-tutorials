...
import type { Chapter, Section } from "../data/javaContent";
...
const JavaChapter = () => {
    ...
    const chapter = javaChapters.find((ch) => ch.slug === slug);
    const chapterIndex = javaChapters.findIndex((ch) => ch.slug === slug);
    const prevChapter = chapterIndex > 0 ? javaChapters[chapterIndex - 1] : null;
    const nextChapter = chapterIndex < javaChapters.length - 1 ? javaChapters[chapterIndex + 1] : null;
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="text-center">
                    ...
                    <Link to="/java" className="text-orange-600 hover:underline">← Back to Java Tutorial</Link>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-white">
            ...
            <div className="bg-gray-50 border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
                    <nav className="flex items-center gap-2 text-sm text-gray-500">
                        ...
                        <Link to="/java" className="hover:text-orange-600 transition-colors">Java</Link>
                        <ChevronRight className="w-3.5 h-3.5" />
            ...
            <div className="max-w-7xl mx-auto flex">
                ...
                <button
                    ...
                    className="lg:hidden fixed bottom-6 left-6 z-50 bg-orange-600 text-white p-3 rounded-full shadow-lg hover:bg-orange-700 transition-colors"
                >
                ...
                <aside className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:sticky top-16 left-0 z-40 w-72 h-[calc(100vh-4rem)] bg-white lg:bg-transparent border-r border-gray-100 overflow-y-auto transition-transform duration-300 lg:block shrink-0`}>
                    <div className="p-4">
                        <Link to="/java" className="flex items-center gap-2 text-sm font-semibold text-orange-600 mb-4 hover:text-orange-700">
                            <BookOpen className="w-4 h-4" /> ☕ Java Tutorial
                        </Link>
                        <nav className="space-y-0.5">
                            {javaChapters.map((ch) => (
                                ...
                                >
                                    <span className={`w-6 h-6 rounded text-xs flex items-center justify-center font-bold shrink-0 ${ch.slug === slug ? "bg-orange-600 text-white" : "bg-gray-100 text-gray-500"
                                        }`}>
                                        {ch.id}
                                    </span>
                ...
                <main className="flex-1 min-w-0 px-4 sm:px-8 lg:px-12 py-8 lg:py-10">
                    ...
                    <div className="mt-12 pt-8 border-t border-gray-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                        ...
                        {nextChapter ? (
                            ...
                            <Link
                                to="/java"
                                className="flex items-center justify-end gap-3 px-5 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-red-500 text-white hover:shadow-md transition-all"
                            >
                                <div className="text-right">
                                    <p className="text-xs text-orange-100">Completed!</p>
                                    <p className="font-semibold">Back to Java Tutorial</p>
                                </div>
                            </Link>
                        )}
                    </div>
