...
import { ChevronRight, ChevronLeft, Home, BookOpen, Copy, Check, AlertTriangle, Lightbulb, Menu, X, Terminal, Globe } from "lucide-react";
...
const SectionContent = ({ section }: { section: Section }) => {
    const renderText = (text: string) => {
        ...
        return parts.map((part, i) => {
            ...
            if (part.startsWith("`") && part.endsWith("`")) return <code key={i} className="bg-gray-100 text-green-800 px-1.5 py-0.5 rounded text-sm font-mono border border-gray-200">{part.slice(1, -1)}</code>;
...
const DjangoChapter = () => {
    ...
    const chapter = allDjangoChapters.find((ch) => ch.slug === slug);
    const chapterIndex = allDjangoChapters.findIndex((ch) => ch.slug === slug);
    const prevChapter = chapterIndex > 0 ? allDjangoChapters[chapterIndex - 1] : null;
    const nextChapter = chapterIndex < allDjangoChapters.length - 1 ? allDjangoChapters[chapterIndex + 1] : null;
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="text-center">
                    ...
                    <Link to="/django" className="text-green-700 hover:underline">← Back to Django Tutorial</Link>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-white">
            <div className="bg-gray-50 border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
                    <nav className="flex items-center gap-2 text-sm text-gray-500">
                        <Link to="/" className="flex items-center gap-1 hover:text-green-700 transition-colors"><Home className="w-3.5 h-3.5" /> Home</Link>
                        <ChevronRight className="w-3.5 h-3.5" />
                        <Link to="/django" className="hover:text-green-700 transition-colors">Django Tutorial</Link>
                        <ChevronRight className="w-3.5 h-3.5" />
            ...
            <div className="max-w-7xl mx-auto flex">
                <button onClick={() => setSidebarOpen(!sidebarOpen)} className="lg:hidden fixed bottom-6 left-6 z-50 bg-green-700 text-white p-3 rounded-full shadow-lg hover:bg-green-800 transition-colors">
                ...
                <aside className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:sticky top-16 left-0 z-40 w-72 h-[calc(100vh-4rem)] bg-white lg:bg-transparent border-r border-gray-100 overflow-y-auto transition-transform duration-300 lg:block shrink-0`}>
                    <div className="p-4">
                        <Link to="/django" className="flex items-center gap-2 text-sm font-semibold text-green-700 mb-4 hover:text-green-800">
                            <BookOpen className="w-4 h-4" /> 🌐 Django Tutorial
                        </Link>
                        <nav className="space-y-0.5">
                            {allDjangoChapters.map((ch) => (
                                <Link key={ch.id} to={`/django/${ch.slug}`} onClick={() => setSidebarOpen(false)}
                                    className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all duration-150 ${ch.slug === slug ? "bg-green-50 text-green-800 font-semibold border border-green-200" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}>
                                    <span className={`w-6 h-6 rounded text-xs flex items-center justify-center font-bold shrink-0 ${ch.slug === slug ? "bg-green-700 text-white" : "bg-gray-100 text-gray-500"}`}>{ch.id}</span>
                ...
                <main className="flex-1 min-w-0 px-4 sm:px-8 lg:px-12 py-8 lg:py-10">
                    <div className="mb-8">
                        <p className="text-sm font-semibold text-green-700 mb-1">Module {chapter.id}</p>
                    ...
                    <div className="mt-12 pt-8 border-t border-gray-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                        {prevChapter ? (
                            <Link to={`/django/${prevChapter.slug}`} className="flex items-center gap-3 px-5 py-3 rounded-xl border border-gray-200 hover:border-green-300 hover:shadow-md transition-all group">
                                <ChevronLeft className="w-5 h-5 text-gray-400 group-hover:text-green-600" />
                                <div><p className="text-xs text-gray-400">Previous</p><p className="font-semibold text-gray-700 group-hover:text-green-700">{prevChapter.title}</p></div>
                            </Link>
                        ) : <div />}
                        {nextChapter ? (
                            <Link to={`/django/${nextChapter.slug}`} className="flex items-center justify-end gap-3 px-5 py-3 rounded-xl border border-gray-200 hover:border-green-300 hover:shadow-md transition-all group text-right">
                                <div><p className="text-xs text-gray-400">Next</p><p className="font-semibold text-gray-700 group-hover:text-green-700">{nextChapter.title}</p></div>
                                <ChevronRight className="w-5 h-5 text-gray-400 group-hover:text-green-600" />
                            </Link>
                        ) : (
                            <Link to="/django" className="flex items-center justify-end gap-3 px-5 py-3 rounded-xl bg-gradient-to-r from-green-700 to-emerald-800 text-white hover:shadow-md transition-all">
                                <div className="text-right"><p className="text-xs text-green-200">Completed!</p><p className="font-semibold">Back to Django Tutorial</p></div>
                            </Link>
                        )}
                    </div>
                </main>
