...
import { k8sChapters } from "../data/k8sContent";
...
const SectionContent = ({ section }: { section: Section }) => {
    return (
        <div className="mb-10">
            ...
            {section.note && (
                <div className="mt-4 bg-indigo-50 border-l-4 border-indigo-500 rounded-r-lg p-4 flex gap-3">
...
const K8sChapter = () => {
    ...
    const chapter = k8sChapters.find((ch) => ch.slug === slug);
    const chapterIndex = k8sChapters.findIndex((ch) => ch.slug === slug);
    const prevChapter = chapterIndex > 0 ? k8sChapters[chapterIndex - 1] : null;
    const nextChapter = chapterIndex < k8sChapters.length - 1 ? k8sChapters[chapterIndex + 1] : null;
        ...
        <div className="min-h-screen bg-white">
            <div className="bg-gray-50 border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
                    <nav className="flex items-center gap-2 text-sm text-gray-500">
                        ...
                        <Link to="/devops" className="hover:text-indigo-600 transition-colors">DevOps</Link>
                        <ChevronRight className="w-3.5 h-3.5" />
                        <Link to="/devops/kubernetes" className="hover:text-indigo-600 transition-colors">Kubernetes</Link>
                        <ChevronRight className="w-3.5 h-3.5" />
            ...
            <div className="max-w-7xl mx-auto flex">
                <aside className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:sticky top-16 left-0 z-40 w-72 h-[calc(100vh-4rem)] bg-white border-r border-gray-100 overflow-y-auto transition-transform lg:block`}>
                    <div className="p-4">
                        <nav className="space-y-0.5">
                            {k8sChapters.map((ch) => (
                                <Link key={ch.id} to={`/devops/kubernetes/${ch.slug}`} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm ${ch.slug === slug ? "bg-indigo-50 text-indigo-800 font-semibold" : "text-gray-600 hover:bg-gray-50"}`}>
                                    <span className={`w-6 h-6 rounded text-xs flex items-center justify-center font-bold ${ch.slug === slug ? "bg-indigo-600 text-white" : "bg-gray-100"}`}>{ch.id}</span>
                ...
                <main className="flex-1 min-w-0 px-4 sm:px-8 lg:px-12 py-8 lg:py-10">
                    <div className="mb-8">
                        <p className="text-sm font-semibold text-indigo-600 mb-1">Chapter {chapter.id}</p>
                    ...
                    <div className="mt-12 pt-8 border-t border-gray-200 flex justify-between gap-4">
                        {prevChapter && <Link to={`/devops/kubernetes/${prevChapter.slug}`} className="flex items-center gap-3 px-5 py-3 rounded-xl border border-gray-200 hover:border-indigo-300 transition-all">
                            <ChevronLeft className="w-5 h-5 text-gray-400" />
                            <div><p className="text-xs text-gray-400">Previous</p><p className="font-semibold text-gray-700">{prevChapter.title}</p></div>
                        </Link>}
                        {nextChapter && <Link to={`/devops/kubernetes/${nextChapter.slug}`} className="flex items-center justify-end gap-3 px-5 py-3 rounded-xl border border-gray-200 hover:border-indigo-300 transition-all text-right">
