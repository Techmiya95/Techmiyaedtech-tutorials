...
import { allCChapters } from "../data/cIndex";
...
const CodeBlock = ({ code, title, output }: { code: string; title?: string; output?: string }) => {
    ...
    return (
        <div className="my-6 rounded-xl overflow-hidden shadow-lg border border-gray-700/50" style={{ background: "#1a1b26" }}>
            ...
            <SyntaxHighlighter language="c" style={customStyle} showLineNumbers={true} wrapLines={true}
...
const SectionContent = ({ section }: { section: Section }) => {
    ...
    return (
        <div className="mb-10">
            ...
            {section.note && (
                <div className="mt-4 bg-blue-50 border-l-4 border-blue-400 rounded-r-lg p-4 flex gap-3">
                    <Lightbulb className="w-5 h-5 text-blue-500 shrink-0 mt-0.5" />
                    <p className="text-sm text-blue-800 leading-relaxed">{section.note}</p>
                </div>
            )}
        </div>
    );
...
const CChapter = () => {
    ...
    const chapter = allCChapters.find((ch) => ch.slug === slug);
    const chapterIndex = allCChapters.findIndex((ch) => ch.slug === slug);
    const prevChapter = chapterIndex > 0 ? allCChapters[chapterIndex - 1] : null;
    const nextChapter = chapterIndex < allCChapters.length - 1 ? allCChapters[chapterIndex + 1] : null;
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="text-center">
                    ...
                    <Link to="/c-programming" className="text-blue-600 hover:underline">← Back to C Programming Tutorial</Link>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-white">
            <div className="bg-gray-50 border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
                    <nav className="flex items-center gap-2 text-sm text-gray-500">
                        <Link to="/" className="flex items-center gap-1 hover:text-blue-600 transition-colors"><Home className="w-3.5 h-3.5" /> Home</Link>
                        <ChevronRight className="w-3.5 h-3.5" />
                        <Link to="/c-programming" className="hover:text-blue-600 transition-colors">C Programming</Link>
                        <ChevronRight className="w-3.5 h-3.5" />
            ...
            <div className="max-w-7xl mx-auto flex">
                <button onClick={() => setSidebarOpen(!sidebarOpen)} className="lg:hidden fixed bottom-6 left-6 z-50 bg-blue-600 text-white p-3 rounded-full shadow-lg hover:bg-blue-700 transition-colors">
                ...
                <aside className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:sticky top-16 left-0 z-40 w-72 h-[calc(100vh-4rem)] bg-white lg:bg-transparent border-r border-gray-100 overflow-y-auto transition-transform duration-300 lg:block shrink-0`}>
                    <div className="p-4">
                        <Link to="/c-programming" className="flex items-center gap-2 text-sm font-semibold text-blue-600 mb-4 hover:text-blue-700">
                            <BookOpen className="w-4 h-4" /> ⚙️ C Programming
                        </Link>
                        <nav className="space-y-0.5">
                            {allCChapters.map((ch) => (
                                <Link key={ch.id} to={`/c-programming/${ch.slug}`} onClick={() => setSidebarOpen(false)}
                                    className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all duration-150 ${ch.slug === slug ? "bg-blue-50 text-blue-700 font-semibold border border-blue-200" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}>
                                    <span className={`w-6 h-6 rounded text-xs flex items-center justify-center font-bold shrink-0 ${ch.slug === slug ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-500"}`}>{ch.id}</span>
                ...
                <main className="flex-1 min-w-0 px-4 sm:px-8 lg:px-12 py-8 lg:py-10">
                    <div className="mb-8">
                        <p className="text-sm font-semibold text-blue-600 mb-1">Module {chapter.id}</p>
                    ...
                    <div className="mt-12 pt-8 border-t border-gray-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                        {prevChapter ? (
                            <Link to={`/c-programming/${prevChapter.slug}`} className="flex items-center gap-3 px-5 py-3 rounded-xl border border-gray-200 hover:border-blue-300 hover:shadow-md transition-all group">
                                <ChevronLeft className="w-5 h-5 text-gray-400 group-hover:text-blue-500" />
                                <div><p className="text-xs text-gray-400">Previous</p><p className="font-semibold text-gray-700 group-hover:text-blue-600">{prevChapter.title}</p></div>
                            </Link>
                        ) : <div />}
                        {nextChapter ? (
                            <Link to={`/c-programming/${nextChapter.slug}`} className="flex items-center justify-end gap-3 px-5 py-3 rounded-xl border border-gray-200 hover:border-blue-300 hover:shadow-md transition-all group text-right">
                                <div><p className="text-xs text-gray-400">Next</p><p className="font-semibold text-gray-700 group-hover:text-blue-600">{nextChapter.title}</p></div>
                                <ChevronRight className="w-5 h-5 text-gray-400 group-hover:text-blue-500" />
                            </Link>
                        ) : (
                            <Link to="/c-programming" className="flex items-center justify-end gap-3 px-5 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:shadow-md transition-all">
                                <div className="text-right"><p className="text-xs text-blue-200">Completed!</p><p className="font-semibold">Back to C Programming</p></div>
                            </Link>
                        )}
                    </div>
                </main>
